 ...
 */
test('OGRS regression test ref 12', async ({ oasys, offender, assessment, ogrs }) => {
    ...
    const offender1: OffenderDef = {
        forename1: 'Autotest',
        surname: 'Auto-MH DATZU',
        pnc: '48/6137555K',
        probationCrn: 'ZVWWTEB',
        gender: 'Male',
    ...
    await oasys.handleAlert('Calculate Predictor Scores', 'Please complete all questions and click Calculate Predictor Scores')
    const ogrsResult = await ogrs.checkOgrsInStandaloneCsrp(pk1)
...
/*
Now complete ALL of the remaining DYNAMIC questions including selecting and populating ALL of the DRUG table and checkboxes and entering in text for other drug (this is for cloning purposes later on)
Click on <Calculate Predictor Scores> - scores have been calculated and the screen has turned READ ONLY
Check the CLOG entries to ensure the front-end has passed the correct values for the input parameters
Scroll down to the Predictors section
    All Reoffending Predictor over the next two yearsSHOWS	DYNAMIC: ‘ ’:OGP2_PERCENTAGE_2YR:’%   ’:risk description
    Violent Reoffending Predictor over the next two yearsSHOWS	DYNAMIC: ‘ ’:OVP2_PERCENTAGE_2YR:’%   ’:risk description
    Serious Violent Reoffending Predictor over the next two yearsSHOWS	DYNAMIC: ‘ ’:SNSV_PERCENTAGE_2YR_DYNAMIC:’%   ’:risk description
    Images and Indirect Contact - Sexual Reoffending PredictorSHOWS	Not Applicable
    Direct Contact - Sexual Reoffending PredictorSHOWS	Not Applicable
    Combined Serious Reoffending PredictorSHOWS	Graph for DYNAMIC CSRP calculation
Take screenshots of the entire read only standalone RSR screen
Close out of the standalone RSR - back to the Offender record
...
Check the OFFENDER_RSR_SCORES record has the existing OGRS 3 fields populated and the new predictor fields populated correctly including original OSP-DC, OSP-IIC and RSR fields
NOW TO CHECK CLONING OF STANDALONE RSR DATA TO AN OASYS ASSESSMENT
...
Navigate to Section 8 and ensure fields and the drug table have been set as per the standalone RSR screenshot NOTE: 8.4, 8.5 and 8.6 will automatically pre-populate and be read only
Navigate to the Predictors screen
Scroll down and ensure that the Predictors section is showing the same DYNAMIC scores as calculated by the standalone RSR
